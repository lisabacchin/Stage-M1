function Modele_2D_rev(muin,Gin,muaft, Gaft, Number, tend, dossier, random)
%mod 
%%%%%%%%%%%%%%%%%%%%%%%
...
nxt = zeros(xdim^2,tdim);
Gxt = zeros(xdim^2,tdim);
muxt = zeros(xdim^2,tdim);
...
N= xdim -1;        % serve se poi faccio dominio non quadrato
dy=dx;
...
for phase = 1:2
    ...
    % seules les concentrations changent
    if phase == 2
        mu(:) = mu0;
        G(:)  = G0;
    ...
    % réinitialisation du schéma BDF2
    ...
    if implicit
        while z <=tend
            ...
            if mod(step,500) == 0
                ...
                % Update title
                title(sprintf('n(x,t) at t = %.2f, cells = %.2f, mu0 = %.3f, G0 = %.3f, mu1 = %.3f, G1 = %.3f, phase = %d  ', z, Number, muin, Gin, muaft, Gaft, phase));
                drawnow;
            end
    ...
    end      % fin de la boucle sur les phases
    ...
    title(sprintf('n(x,t) at t = %.2f, cells = %.2f, mu0 = %.3f, G0 = %.3f, mu1 = %.3f, G1 = %.3f, Phase = %d  ', tend, Number, muin, Gin, muaft, Gaft, phase));%, 'FontSize', 14, 'FontWeight', 'bold');
    xlabel('X');
    ylabel('Y');
...
filename = sprintf('mu0_%0.2f_G0_%0.2f_mu1_%0.2f_G1_%0.2f_t_%d_cells_%d_Phase_%d', muin, Gin,muaft, Gaft, tend, Number, phase);
